...
/// <summary>
/// Typed JSON converter for objects containing <see cref="SecureIdAttribute"/> properties.
...
public sealed class SecureIdJsonConverter<T> : JsonConverter<T> where T : class
{
    ...
    public SecureIdJsonConverter(ISecureIdService service, ISecureIdOutboundStringTransform transform)
    {
        _service = service;
    ...
    public override T? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        while (reader.Read())
        {
            ...
            if (meta.SecureIdAttr is not null)
                ReadSecureIdProperty(ref reader, result, meta);
            else
    ...
    public override void Write(Utf8JsonWriter writer, T value, JsonSerializerOptions options)
    {
        ...
        foreach (var meta in metadata)
        {
            ...
            if (meta.SecureIdAttr is not null)
                WriteSecureIdValue(writer, propValue, meta.Property.PropertyType);
    ...
    private void ReadSecureIdProperty(ref Utf8JsonReader reader, T target, PropMeta meta)
    {
    ...
    private void WriteSecureIdValue(Utf8JsonWriter writer, object? value, Type propertyType)
    {
        if (value is null)
        {
            writer.WriteNullValue();
    ...
    private static PropMeta[] GetMetadata() =>
        MetadataCache.GetOrAdd(typeof(T), static type =>
            type.GetProperties(BindingFlags.Public | BindingFlags.Instance)
                ...
                {
                    Property = p,
                    SecureIdAttr = p.GetCustomAttribute<SecureIdAttribute>(),
                    IsIgnored = p.GetCustomAttribute<JsonIgnoreAttribute>() is not null,
                    ExplicitJsonName = p.GetCustomAttribute<SecureIdAttribute>()?.JsonPropertyName
    ...
    private sealed class PropMeta
    {
        ...
        public SecureIdAttribute? SecureIdAttr { get; init; }
